...
int main(int argc, char *argv[])
{
    ...
    int segmentSize = sizeof(int) + (segmentsNum * sizeof(SegmentLock)) + sizeof(Server::Statistics) + sizeof(MySemaphore);
    ...
    int blockIndex = GetBlockNumber(recordId, editor, segmentsNum) - 1;
    if (blockIndex < 0) { return EXIT_FAILURE; }
    ...
    void* placementAddress = reinterpret_cast<char*>(segmentMem) + blockIndex * sizeof(SegmentLock);
    SegmentLock* segmentLock = reinterpret_cast<SegmentLock*>(placementAddress);

    segmentLock->WriterLock();

    t2 = (double)times(&tb2);
    double realWaitTime = (t2 - t1) / ticspersec;

    Account account = editor.ReadRecordById(recordId);
    if (account.Balance + value < 0)
    {
        cout << "Record: " << recordId << " current balance: " << account.Balance << " is not enough to withdraw: " << -value << endl;
        snprintf(buffer, sizeof(buffer), "Record: %d current balance: %.d is not enough to withdraw: %d\n", recordId, account.Balance, -value);
    ...
    editor.WriteRecordById(recordId, account);
    sleep(time);

    cout << "Record's : " << recordId << ", balance changed by: " << value << endl;
    snprintf(buffer, sizeof(buffer), "Record's : %d, balance changed by: %d\n", recordId, value);
    SafeLogToFile(logFd, buffer);

    cout << "Id: " << account.CustId << " Name: " << account.FirstName << " Lastname: " << account.LastName << " New Balance: " << account.Balance << endl << endl;
    snprintf(buffer, sizeof(buffer), "Id: %d Name: %s Lastname: %s New Balance: %d\n\n", account.CustId, account.FirstName, account.LastName, account.Balance);
    SafeLogToFile(logFd, buffer);

    segmentLock->WriterUnlock();
    ...
    sem->Wait();
    statistics->NumOfWriters++;
    statistics->NumOfRecordsProcessed++;
    statistics->TotalWritterTime += realActiveTime;
...
int GetBlockNumber(int custId, RecordEditor& editor, int numOfSegments)
{
    ...
    if (numRecords <= 0)
    {
        cerr << "Writer did not find accounts in file" << endl;
        return -1;
    }
...
void SafeLogToFile(int fd, const char* data)
{
    ...
    int length = snprintf(message, sizeof(message), "%s.%03d - Writter (PID: %d) --> %s", time_str, millis, pid, data);
